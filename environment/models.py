...
class Observation(BaseModel):
    ...
    reward: float = Field(
        default=0, ge=0.0, le=1.0, description="Cumulative reward"
    )
...
class Reward(BaseModel):
    ...
    value: float = Field(default=0, ge=0.0, le=1.0, description="Total reward value")
    partial_credit: float = Field(
        default=0, ge=0.0, le=1.0, description="Credit for partial test passes"
    )
    penalty: float = Field(
